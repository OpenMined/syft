...
async def handle_post(request):
    ...
    try:
        ...
        if "data" in body and "metrics" in body["data"]:
            ...
            combined_df["date"] = combined_df["date"].str.replace(r"(\d{2}:\d{2}:\d{2})(\+\d{4})", r"\1 \2", regex=True)
            ...
            response = {"message": "Filtered, sorted, and appended data saved to metrics_data.csv"}
