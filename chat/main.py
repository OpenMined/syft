...
client_config = Client.load("/Users/madhavajay/dev/syft/.clients/bigquery@openmined.org/config/config.json")
...
def sandbox_path(file_path):
    ...
    if not abs_path.is_relative_to(client_config.datasite_path):
        logger.warning(f"Attempted access to {abs_path} outside sandbox {client_config.datasite_path}")
        raise ValueError(
